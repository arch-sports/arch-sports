...
subtitle: \huge `r gsub("_", "/", params$Team)`
...
data <- read.csv(file = './Data/basketball_random.csv') %>%
  mutate(CMJ = CMJ*2.54, Team = gsub('/', '_', Team)) 

data <- data[data$Team == params$Team, ]
...
ggplot(data, mapping = aes(x = FMS)) +
  geom_histogram(bins = length(unique(data$FMS)), color = 'black', fill = 'red', alpha = 0.5) +
